...
io.on("connection", (socket) => {
    ...
    socket.on('send', message => {
        ...
        // Output: Map(2) { '4wy5BtpzatnWhwd8AAAG' => 'asdfd', 'tOWhu1jVHJe_EpGjAAAR' => 'sandy' }
        ...
        if (recipientName != undefined) {
            ...
            Object.entries(users).forEach(([key, value]) => {
                ...
                if (value == recipientName) {
                    recipientId = key
                    io.to(recipientId).emit('receive', { message, name: users[socket.id], time: timestamp, userId: message.recipientName });
